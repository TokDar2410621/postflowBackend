...
def facebook_callback(request):
    ...
    if error:
        return redirect(f"{frontend}/profile?facebook_error={error}")

    cached = cache.get(f'facebook_oauth:{state}')
    if not cached:
        return redirect(f"{frontend}/profile?facebook_error=invalid_state")
    ...
    if not token_resp.ok:
        ...
        return redirect(f"{frontend}/profile?facebook_error=token_failed")
    ...
    if not me_resp.ok:
        return redirect(f"{frontend}/profile?facebook_error=userinfo_failed")
    ...
    FacebookAccount.objects.update_or_create(
        ...
        defaults={
            'user': django_user,
            'name': me.get('name', ''),
            'profile_picture_url': me.get('picture', {}).get('data', {}).get('url', ''),
            'access_token': access_token,
            'page_id': page_id,
